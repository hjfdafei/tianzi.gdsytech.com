...
class OrdersService extends Base{
    ...
    public function orders_exportdata($map,$field='o.*'){
        ...
        $listdata=$list['list'];
        $filename='订单信息表';
        $head=['所在校区','订单类型','订单号','姓名','联系电话','身份证号码','院系','学号','宿舍地址','推荐人','办理网格','用户自输入/续费宽带账号','宽带套餐','系统分配宽带账号','系统分配宽带密码','宽带有效期','校园通讯卡号码','应付金额','优惠金额','实付金额','支付时间','订单状态','下单时间'];
        $data=[];
        ...
        foreach($listdata as $v){
            ...
            $data[]=[$schoolname,$stylename,"\t".$v['orderno'],$v['realname'],"\t".$v['mobile'],"\t".$v['idcardnum'],$v['department'],"\t".$v['studentnumber'],$v['address'],$v['promoter'],$v['work_grid'],$v['broadband_account'],$v['goods_title'],$v['keyaccount'],$v['keypassword'],"\t".$start_time.'--'."\t".$end_time,"\t".$v['school_mobile'],$money,$discount_money,$pay_money,"\t".$v['pay_time'],$statusname,"\t".$v['create_time']];
        }
