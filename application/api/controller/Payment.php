            ...
            if(strtolower($checkres['result_code'])=='success' && strtolower($checkres['return_code'])=='success' && strtolower($checkres['trade_state'])=='success'){
                ...
                if(!empty($info)){
                    if($info['ispay']!=1){
                        ...
                        if($info['orders_style']==1){
                            $bmap=[];
                            $bmap[]=['status','=',1];
                            $bmap[]=['isuse','=',2];
                            $bmap[]=['school_id','=',$info['school_id']];
                        ...
                        }elseif($info['orders_style']==2){
                            $bmap=[];
                            $bmap[]=['keyaccount','=',$info['broadband_account']];
                            $hasbroadband=$uservice->checkBroadband($bmap);
                            if(empty($hasbroadband)){
                                ...
                                $bdata=[
                                    ...
                                    'keyaccount'=>$info['broadband_account'],
                                    'keypassword'=>'',
                                    'create_time'=>date('Y-m-d H:i:s'),
                                    'addpath'=>2,
                                ];
                                $up_broadband_id=$uservice->addBroadband($bdata);
                            }else{
                                $up_broadband_id=$hasbroadband['id'];
                            }
                            $oumap=[];
                            $oumap[]=['id','=',$info['id']];
                            $oudata=[];
                            $oudata['broadband_id']=$up_broadband_id;
                            $oudata['status']=3;
                            $oudata['finish_time']=date('Y-m-d H:i:s');
                            $oudata['update_time']=date('Y-m-d H:i:s');
                            $res=DB::name('orders')->where($oumap)->update($oudata);
                            send_mini_broadbandtpl($info['openid'],'宽带续费',$info['realname'],round($info['money']/100,2),$info['id']);
                        }
                    }
                }
            }
