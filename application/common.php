...
function validation_idcard($id_card){//只检测18位身份证号码
    ...
    if(strlen($id_card)==18){
        ...
        //台湾10位 例：U193683453
        //澳门8位  例：5686611(1)
        $hkmatch="/[A-Za-z]{1}\\d{6}[(\\d)|A]{3}/";
        $twmatch="/[A-Z]{1}\\d{9}/";
        $macaumatch="/[1|5|7]\\d{6}[(\\d)]{3}/";
        if(preg_match($hkmatch,$id_card)){
            return true;
        }else{
            if(preg_match($macaumatch,$id_card)){
                return true;
            }else{
                if(preg_match($twmatch,$id_card)){
                    return true;
                }else{
                    return false;
                }
